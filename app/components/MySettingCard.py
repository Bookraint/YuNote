...
class ElidedLabel(QLabel):
    """宽度不足时自动省略文字（末尾显示 ...），完整内容以 tooltip 展示。"""

    def __init__(self, text: str = "", parent=None):
        super().__init__(text, parent)   # 先用原始文字正常显示
    ...
    def setText(self, text: str):
        ...
        if text:
            self.setToolTip(text)
        # 宽度已知时立即省略，否则先显示完整文字等 resizeEvent 再更新
        self._updateElided()
    ...
    def _updateElided(self):
        ...
        if not self._full_text:
            ...
            return
        if w <= 0:
            # 还没完成布局，先保留完整文字，等 resizeEvent 触发后再省略
            super().setText(self._full_text)
            return
        elided = self.fontMetrics().elidedText(
            self._full_text, Qt.ElideRight, w  # type: ignore
        )
...
class SettingCard(QFrame):
    ...
    def __init__(
        ...
        self.contentLabel = ElidedLabel(content or "", self)
